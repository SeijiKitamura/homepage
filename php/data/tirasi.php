...
foreach($title["data"] as $key=>$val){
 //リンク作成
 $url="tirasi.php?tirasi_id=".$val["tirasi_id"]."&hiduke=".$val["hiduke"];

 //日付
 $d=date("n月d日",strtotime($val["hiduke"]));
 //リスト作成
 $html.="<li>";
 if($val["tirasi_id"]!=$tirasi_id) $html.="<a href='".$url."'>";
 $html.=$d." ".$val["title"];
 if($val["tirasi_id"]!=$tirasi_id) $html.="</a>";
 $html.="</li>\n";
}//foreach
echo $html;
?>
